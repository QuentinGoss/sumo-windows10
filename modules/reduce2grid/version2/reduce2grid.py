    ...
    for edge in root.findall('edge'):
        ...
        if xml_has_atrribute(edge,'shape'):
            ...
            for x0y0x1y1 in child_edges:
                ...
                eid = edge.attrib['id']
                add_connection(eid,p_from,p_to)
                continue
...
class Connection(object):
    def __init__(self,ifrom,ito):
        ...
        self.edges = []      # The edges that exist as part of this connection.
...
def add_connection(eid,p_from,p_to):
    ...
    if check4anomaly(ifrom,ito):
        ...
        for conn in child_connections:
            ...
            validate_and_add_connection(eid,ifrom,ito)
            continue
...
def validate_and_add_connection(eid,ifrom,ito):
    # Grab Cell
    ...
    if len(cell.connections) == 0:
        ...
        cell.connections[0].edges.append(eid)
    ...
    # See if a connection exists
        ...
        for conn in cell.connections:
            if conn.compare(ifrom,ito):
                ...
                if not eid in conn.edges: conn.edges.append(eid)
                break
            continue
        if not found:
            cell.connections.append(Connection(ifrom,ito))
            cell.connections[-1].edges.append(eid)
...
# Write the .conn.csv file
# @param options = Command line arguments
def write_conn_csv(options):
    # Statistics tracking
    n = 1; total = GRID.ncols * GRID.nrows; n_conn = 0
    
    # Write to file
    with open('./temp/%s.conn.csv' % (options.name),'w') as csv:
        csv.write('CONNECTION ID,# EDGES,EDGE IDS\n')
        for col in GRID.cells:
            for cell in col:
                for conn in cell.connections:
                    if len(conn.edges) > 0:
                        csv.write('road%d,%d' % (n_conn,len(conn.edges)))
