    ...
    for s_line in net_xml:
      if '<edge' in s_line:
        if 'id="' in s_line:
            ...
            collect_edge_info(is_first,s_id_from,s_id_to,_id)
            is_first = False
...
def collect_edge_info(is_first,s_id_from,s_id_to,_id):
  s_junctions_json = "junctions.json"
  ...
  with open(s_junctions_json,"r") as junctions_json:
    for s_line_jj in junctions_json:
      if is_complete_to and is_complete_from:
        ...
        combine_and_write_edge_json(is_first,s_id_to, s_id_from, s_true_coords_from, s_true_coords_to, s_normal_coords_to, s_normal_coords_from,_id)
        break
...
def combine_and_write_edge_json(is_first, s_id_to, s_id_from, s_true_coords_from, s_true_coords_to, s_normal_coords_to, s_normal_coords_from,_id):
  s_edges_json = "edges.json"
  with open(s_edges_json,"a") as edges_json:
    if not is_first:
      edges_json.write(',\n')
    #edges_json.write('\t"' + s_id_from + '_to_' + s_id_to + '": {\n')
    edges_json.write('\t"' + _id + '": {\n')
