...
def sink_from_prob(f_rand,lf_prob):
  ...
  f_val = 0.0
  llsf_sink_prob = []
  for i in range(len(config.sink.ids)):
    if lf_prob[i] == 0.0: continue
    llsf_sink_prob.append([config.sink.ids[i],f_val])
    f_val += lf_prob[i]
  # end for
  del f_val
...
def spawn_tts(traci,n_step,N_VEHICLES,random):
  ...
  if len(traci.vehicle.getIDList()) < config.n_vehicles_max:
    for i in range(len(config.ls_ttir)):
      if n_step % config.ttsr.rate[i] == 0:
        f_rand = random.random()
        s_sink = sink_from_prob(f_rand,config.prob_sink.prob[i])
